...
solvDif_CN_Neumann(double **q, double *w, double Lz, double dt, int nt, int nz)
{
 ...
 coef=(dt*(double)(nz*nz))/(6.0*Lz*Lz);
 coef1=(dt*(double)(nz*nz))/(12.0*Lz*Lz);
...
{
  int i, is, iG,j;
  double Qc, Qs,Test;  /* the single chain partition function */
 ...
 {  
  ...
  /*printf("%.12e \t %.12e \t %.12e \t %.12e\n",s->wA[i],s->wB[i],s->wShiftA[i],s->wShiftB[i]);*/
 }
 ...
 {  
  ...
  /*n = (s->GL_abscissa[iG]*s->Nn)/(double)(s->GL_poly);*/
  ...
  /*s->Qc_av = s->Qc_av + (s->XP*exp(s->mu_p*n - s->wAvg*n)*s->GL_Qb[iG]*s->GL_weight[iG])/(s->GL_Gamma*(double)s->Nn);*/
  s->Qc_av = s->Qc_av + (s->XP*exp(s->mu_p*n - s->wAvg*n)*s->GL_Qb[iG])/((double)s->Nn);
 ...
 /*calculate polymer density under potential w*/
  ...
  for (i=0; i<nx+1; i++)
  {
   ...
   /*s->GL_phiA[iG][i] *= (s->XP*exp(s->mu_p*n - s->wAvg*n)*dt*s->GL_weight[iG])/(s->GL_Gamma*(double)s->Nn);*/
   s->GL_phiA[iG][i] *= (s->XP*exp(s->mu_p*n - s->wAvg*n)*dt)/((double)s->Nn);
  ...
  for (i=0; i<nx+1; i++)
  {
   ...
   /*s->GL_phiB[iG][i] *= (s->XP*exp((s->mu_p - s->wAvg)*n)*dt*s->GL_weight[iG])/(s->GL_Gamma*(double)s->Nn);*/
   /*s->GL_phiB[iG][i] *= (s->XP*exp(s->mu_p*n - s->wAvg*n)*dt*s->GL_weight[iG])/(s->GL_Gamma*(double)s->Nn);*/
   s->GL_phiB[iG][i] *= (s->XP*exp(s->mu_p*n - s->wAvg*n)*dt)/((double)s->Nn);
 ...
 for (i=1; i<nx; i++)
 {
   Qs += coef*pow(i,2.0)*exp(-s->wS[i]);
 }
 Qs += 0.5*(coef*pow(0.0,2.0)*exp(-s->wS[0]) + coef*pow(nx,2.0)*exp(-s->wS[nx]) );
 Qs *= (s->Lx/(double)s->nx);
 s->Qs=Qs;
 for (i=0; i<nx+1; i++)
 {
  s->phiS[i] = s->mu_s*exp(-s->wS[i]);
 }
