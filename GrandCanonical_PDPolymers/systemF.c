...
void initialiseSystem(systemF *s)
{
 ...
 double x,y;
 FILE *fp1,*fp2;
 fp1 = fopen("DensInp.dat","r");   /*To initialise densities from a file*/
 fp2 = fopen("Qp_NvsN.dat","r");   /*File with Exp of chem.Pot for Pol with diff chain lengths*/ 
 ...
 s->Emu_p = array1(s->GL_nG);
 for(iG=0;iG<s->GL_nG;iG++)
 {
  fscanf(fp2,"%lf %lf",&x,&y);
  s->Emu_p[iG]=1.0/y;
 }
 fclose(fp2);
