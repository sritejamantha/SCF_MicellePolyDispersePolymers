...
{
 char name[1000];
 sprintf(name,"phiP%.2f_N%.2f_f%.1f_xAB%.0f_xAS%.0f_xBS%.0f_kappa%.0f_Lx%.0f_nx%d",s->XP,s->Nscale,s->f,s->xAB,s->xAS,s->xBS,s->kappa,s->Lx,s->nx);
...
dataOutputFile(systemF *s, double runtime)
{
 ...
 fprintf(fp,"the free energy of the system is %.15e\n", s->freeEnergy);
 fprintf(fp,"\n");
 fprintf(fp,"\n");
 fclose(fp);
